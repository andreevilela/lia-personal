...
{$sql = "select pagamentos.id, alunos.nome, pagamentos.vencimento, pagamentos.status, pagamentos.observacao, pagamentos.pagamento from pagamentos inner join alunos on pagamentos.fk_aluno = alunos.id where pagamentos.id=$cod";
                            ...
                            while($l = mysql_fetch_array($querybanco)){
								...
								$venc = date("m/Y", strtotime($vencimento));
  ...
  <head>
	...
	<table width='850'>
			<tr>
			  ...
			  <td></td>
			  <td><font face=\"face='Arial' size='7'\"><strong><p><center>Recibo</center></p></font> <font face=\"face='Arial' size='4'\"><br> $firma <br>CNPJ: $cnpj </strong></font></td>
			  <td></td>
			</tr>
	</table>
	<hr><br>
<p align='right'><font style='font-family:Verdana, Arial, Helvetica, sans-serif'>RECIBO: R$ ______________ </font></p>
<p align='justify'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font style='font-family:Verdana, Arial, Helvetica, sans-serif'>Recebemos de <strong> $nome </strong> a mensalidade do mês/ano <strong>$venc</strong>, referente às <strong>atividades esportivas (Academia)</strong>. </font></p>
