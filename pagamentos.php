...
;						  else
						    {echo"<li class='cinzaEscuro'> <a href='novopagamento.php'>&nbsp;Novo&nbsp;</a> </li>";} ?> 
                            </li>
                            <li class="cinzaEscuro"> <a href='recebimentos.php'>Recebimentos</a> </li>
            			</ul>
<br>
        			<form class="form-horizontal" role="form" method="post" action="">
                        <div class="form-group">
                            <div class="col-sm-4"> 
                            	<input type="text" name="busca" class="form-control" id="busca" value="<?php if(isset($_POST['busca'])) echo $_POST['busca']; ?>" onfocus="this.style.backgroundColor='#ffe6e6'" autofocus placeholder="Digite sua busca">
                            </div>
<br>
                        </div>
                   <!-- <div class="form-group">
    ...
    <br>                    
                    ...
                    <li class="active">
						...
						if(isset($_POST['busca'])) 
								...
								<li class='cinzaEscuro'>
								   	<a href='exportarpagamentos.php?id=$busca'>Exportar</a>
								</li>";} 
							  else 
								{echo"<a>Pagamentos a Receber</a>
								<li class='cinzaEscuro'>
								   	<a href='exportarpagamentos.php'>Exportar</a>
								</li>";};?>
                    </li>
                	</ul>
                	...
                	<table class="pequena">
						...
						if((isset($_POST['busca'])) && (!empty($_POST['busca'])) && (!empty($_POST['filtro1'])) && (!empty($_POST['filtro2'])))
							{$comando = "where alunos.status ='Ativo' and nome like '%".$aluno."%' and vencimento between '$filtro1' and '$filtro2' order by pagamentos.vencimento";}
						else if((isset($_POST['busca'])) && (!empty($_POST['filtro1'])) && (!empty($_POST['filtro2'])))
							{$comando = "where alunos.status ='Ativo' and vencimento between '$filtro1' and '$filtro2' order by pagamentos.vencimento";}
						else if((isset($_POST['busca'])) && (!empty($_POST['busca'])) && (!empty($_POST['filtro1'])))
							{$comando = "where alunos.status ='Ativo' and vencimento = '$filtro1' order by pagamentos.vencimento";}	
							...
							{$comando = "where alunos.status ='Ativo' and pagamentos.status = 0 and pagamentos.vencimento < '$data_hoje' order by pagamentos.vencimento";}
						...
						$contp = 0;
                            ...
                            {$sql = "select pagamentos.id, alunos.nome, pagamentos.vencimento, pagamentos.status, pagamentos.observacao from pagamentos inner join alunos on pagamentos.fk_aluno = alunos.id $comando";
